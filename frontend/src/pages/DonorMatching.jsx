...
const DonorMatching = () => {
    ...
    return (
        <div className="max-w-7xl mx-auto p-4 md:p-8 grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in duration-700">
            ...
            {/* LEFT: MAP VIEW */}
            <div className="h-[450px] lg:h-[650px] rounded-[48px] overflow-hidden shadow-2xl border-8 border-white sticky top-24 z-10">
            ...
            <div className="space-y-6">
                <div className="flex justify-between items-center bg-white p-6 rounded-[32px] shadow-sm border border-gray-100">
                    <div>
                        <h2 className="text-3xl font-black text-gray-800 tracking-tighter italic text-red-600 uppercase">Heroes Found</h2>
                        <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1">Requesting {data.request.blood} Group</p>
                    </div>
                ...
                <div className="grid gap-5">
                    {data.matches.length > 0 ? data.matches.map(donor => (
                        <div key={donor.unique_id} className="bg-white p-6 rounded-[40px] border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-300 group">
                            <div className="flex justify-between items-start">
                                <div className="flex gap-4">
                                    <div className="bg-slate-50 w-16 h-16 rounded-3xl flex items-center justify-center group-hover:bg-red-50 transition border border-transparent group-hover:border-red-100">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex flex-col gap-1 mt-2">
                                            ...
                                            <div className="flex items-center gap-3 mt-1">
                                                <span className="text-[9px] font-black text-gray-400 flex items-center gap-1 uppercase bg-slate-50 px-2 py-1 rounded-lg">
                                                    <MapPin size={10} className="text-red-500"/> {donor.distance} KM AWAY
                                                </span>
                                                <span className="text-[9px] font-black text-gray-400 flex items-center gap-1 uppercase bg-slate-50 px-2 py-1 rounded-lg border-l-2 border-green-500">
                                                    <Activity size={10} className="text-green-500"/> {donor.healthScore}% HEALTH
                                                </span>
                                            </div>
                                            ...
                                            <div className="flex items-center gap-2 mt-2">
                                                ...
                                                <span className="text-xs font-black text-slate-400 tracking-widest italic">{donor.phone}</span>
                                                <span className="text-[8px] font-black bg-slate-800 text-white px-2 py-0.5 rounded-full tracking-widest uppercase opacity-20">Securely Masked</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div className="text-center">
                                    ...
                                    <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest italic leading-none">Match</p>
                                </div>
                            </div>

                            <div className="mt-6 flex items-center justify-between border-t border-dashed border-gray-100 pt-5">
                                <p className="text-[9px] font-bold text-gray-300 tracking-[0.2em] uppercase italic">LifeDrop ID: #{donor.unique_id}</p>
                                <button 
                                    onClick={() => sendRequest(donor.unique_id)}
                                    className="bg-slate-900 text-white px-10 py-3.5 rounded-[20px] font-black text-xs shadow-xl flex items-center gap-2 hover:bg-red-600 transition-all duration-300 transform active:scale-95 shadow-slate-200 uppercase tracking-widest"
                                >
                        ...
                        <div className="bg-white p-20 rounded-[48px] border-2 border-dashed border-gray-100 text-center flex flex-col items-center animate-pulse">
                            <ShieldCheck size={60} className="text-gray-100 mb-4" />
                            <p className="text-gray-400 font-black uppercase tracking-widest">No compatible donors found nearby</p>
                            <p className="text-[10px] text-gray-300 mt-2 italic">Try increasing the search radius in settings</p>
                        </div>
                    )}
                </div>
            </div>
